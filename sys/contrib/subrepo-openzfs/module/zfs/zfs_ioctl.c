...
history_str_get(zfs_cmd_t *zc)
{
	...
	if (copyinstr((const void *)(__uintcap_t)zc->zc_history,
...
get_nvlist(__uintcap_t nvl, uint64_t size, int iflag, nvlist_t **nvp)
{
	...
	if (ddi_copyin((void *)nvl, packed, size, iflag) != 0) {
...
put_nvlist(zfs_cmd_t *zc, nvlist_t *nvl)
{
	...
	if (size > zc->zc_nvlist_dst_size) {
		...
		if (ddi_copyout(packed, (void *)(__uintcap_t)zc->zc_nvlist_dst,
		    size, zc->zc_iflags) != 0)
...
zfs_ioc_pool_get_history(zfs_cmd_t *zc)
{
	...
	if ((error = spa_history_get(spa, &zc->zc_history_offset,
		...
		error = ddi_copyout(hist_buf,
		    (void *)(__uintcap_t)zc->zc_history,
...
zfs_ioc_error_log(zfs_cmd_t *zc)
{
	...
	error = spa_get_errlog(spa, (void *)(__uintcap_t)zc->zc_nvlist_dst,
...
zfs_ioc_userspace_many(zfs_cmd_t *zc)
{
	...
	if (error == 0) {
		error = xcopyout(buf,
		    (void *)(__uintcap_t)zc->zc_nvlist_dst,
		    zc->zc_nvlist_dst_size);
	}
